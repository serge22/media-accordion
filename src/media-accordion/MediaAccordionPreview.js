...
export default function MediaAccordionPreview({ clientId }) {
    const { selectedBlockId, itemBlocks } = useSelect(
        (select) => {
            ...
            return {
                ...
                itemBlocks: block ? block.innerBlocks.filter(
    ...
    if (itemBlock.attributes.mime && itemBlock.attributes.mime.startsWith('video/')) {
        return (
            <video key={itemBlock.clientId} src={itemBlock.attributes.mediaUrl} controls style={{ maxWidth: '100%', height: 'auto' }} />
        );
    }
    if (itemBlock.attributes.mime && itemBlock.attributes.mime.startsWith('image/')) {
        return (
            <img key={itemBlock.clientId} src={itemBlock.attributes.mediaUrl} alt="Media preview" style={{ maxWidth: '100%', height: 'auto' }} />
        );
    }
